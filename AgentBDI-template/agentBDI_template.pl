...
high_priority(get(Obj), 'Me alcanza la energia para obtener objetos'):-
				...
				property([agent, me], life, St),
                                ...
                                buscar_plan_desplazamiento(Metas,_,NodoMeta,CostoAMeta),
                                 findall(ObjPos, (
                                                at([inn,_], ObjPos)),
		                           Posadas),
                                calcular_costo(NodoMeta,Posadas,CostoVolver),

                                Costo is CostoAMeta + CostoVolver + 20,
...
select_intention(open(Obj), 'Es la tumba mas cercana para abrir', Desires):-
			findall(ObjPos, (member(open(Obj), Desires),
			 at(Obj, ObjPos)),
		Metas), % Obtengo posiciones de todas las tumbas.
...
/*
select_intention(rest, 'Tengo que recargar energi si no me  muero', Desires):-
	member(rest, Desires).
*/
